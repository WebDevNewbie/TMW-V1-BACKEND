<?php defined('BASEPATH') OR exit('No direct script access allowed');
	session_start();
	class Kiosk_dine_in extends Admin_Controller {
	    	...
	    	if($this->session->userdata('logged_in'))
   			{
				...
				$this->load->view('user_dine_in/home', array(
					'data'			=> $data,
					'subview'		=> $this->data,
					'category'		=> $category,
	    	...
	    	if($this->session->userdata('logged_in'))
   			{
				...
				if(count($subcat) > 0)
				{
					...
					$this->load->view('user_dine_in/subcat', array(
						'data'			=> $data,
						'subview'		=> $this->data,
						'category'		=> $subcat,
				...
				elseif (count($item) > 0) 
				{
					...
					$this->load->view('user_dine_in/select_item', array(
						'data'			=> $data,
						'subview'		=> $this->data,
						'category'		=> $item,
						'cat_id'		=> $cat_id,
	    ...
	    {
	    	if((int)$this->input->post('table_no') > 0)
	    	{
	    		$session_order = $this->session->userdata('order_items');
	    		$member = $this->session->userdata('member');
	    		$tmp_price = '';

	    		$query = $this->db->query('SELECT id FROM customer_order_info WHERE customer_order_info.table_no = '.$this->input->post('table_no').' AND status = "current" AND bill_status = "1" ORDER BY order_date DESC LIMIT 1');
				$res = $query->result();

				$query1 = $this->db->query('SELECT * FROM customer_order_info WHERE customer_order_info.table_no = '.$this->input->post('table_no').' AND status = "current" AND bill_status = "0" ORDER BY order_date DESC LIMIT 1');
				$res1 = $query1->result();
				// dump_exit($res1);
				...
				if(count($res1) > 0 )
				{
					foreach ($session_order as $row) 
		    		{
						if(count($row) > 1)
						{
							...
							$tmp_price += $row['item_price']*$row['item_qty'];
					...
					$update_price = array(
							'total_payment'	=> $tmp_price+$res1[0]->total_payment,
							);
					$this->db->where('id', $res1[0]->id);
					$this->db->update('customer_order_info', $update_price);
					...
					if($add_order)
					{
						...
						// dump_exit($tmp_price);
						redirect('kiosk_dine_in','refresh');
					}
					else
					{
	    	...
	    	{
	    		$this->session->set_flashdata('alerts', array('note' => 'Please choose your table number.'));
	    		redirect('kiosk_dine_in','refresh');
	    	}
	    }
